...
def main():
    ...
    while True:
        ...
        if tmr % 200 == 0:  # 200フレームに1回，敵機を出現させる
            emys.add(Enemy())
        if (score.value - 200) // 100 > boss_count:  # 初期スコアは200ポイントで、追加100ポイントごとにボスが生成されます。
            boss_health = 5 + ((score.value - 200) // 100) * 5  # スコアに応じてボスのHPが増加
            bosses.add(Boss(health=boss_health))
